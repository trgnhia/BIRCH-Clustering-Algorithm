...
from mpl_toolkits.mplot3d import Axes3D  # cho 3D scatter
...
OUT_DIR = BASE_DIR / "output_demographics_3D"
...
def pca_scatter_3d(X_scaled, labels, title):
    """Vẽ PCA scatter 3D với màu theo labels."""
    ...
    sc = ax.scatter(
        ...
        c=labels, cmap=cmap_custom,
        s=30, alpha=0.7, edgecolors="none"
    )
    cbar = plt.colorbar(sc, ax=ax, shrink=0.6)
    cbar.set_label("Label")
    ax.set_title(f"{title} — PC1 {var[0]:.1f}%, PC2 {var[1]:.1f}%, PC3 {var[2]:.1f}%")
    ax.set_xlabel("PC1"); ax.set_ylabel("PC2"); ax.set_zlabel("PC3")
    plt.tight_layout(); plt.show()

def pca_scatter_cf_l1_as_points_3d(cf_summary_l1_with_l2, title):
    """Vẽ PCA scatter 3D cho CF L1 centroid, màu = L2_Label."""
    ...
    sc = ax.scatter(
        X_pca[:,0], X_pca[:,1], X_pca[:,2],
        c=labels, cmap=cmap_custom,
        s=80, alpha=0.9, edgecolors="k"
    )
    cbar = plt.colorbar(sc, ax=ax, shrink=0.6)
    cbar.set_label("L2_Label")
    ax.set_title(f"{title} — PC1 {var[0]:.1f}%, PC2 {var[1]:.1f}%, PC3 {var[2]:.1f}%")
    ax.set_xlabel("PC1"); ax.set_ylabel("PC2"); ax.set_zlabel("PC3")
    plt.tight_layout(); plt.show()
...
# PCA scatter 3D
pca_scatter_3d(X_scaled, df_cf_l1["CF_Label"], "BIRCH Micro-Clusters (Demographics)")
df_cf_l1 = df_cf_l1.merge(
    df_l1_with_l2[["CF_Label", "L2_Label"]].drop_duplicates(),
    on="CF_Label", how="left"
)
pca_scatter_3d(X_scaled, df_cf_l1["L2_Label"], "BIRCH Super-Clusters (Demographics)")
pca_scatter_cf_l1_as_points_3d(df_l1_with_l2, "BIRCH Super-Clusters (Demographics) [mỗi chấm = CF L1]")
# =========================
...
print("Explained variance (%):", explained.tolist())  # [PC1%, PC2%, PC3%]
...
corr_pc = Z.join(scores).corr().loc[FEATURES, ["PC1_score", "PC2_score", "PC3_score"]]
print("\n=== TƯƠNG QUAN (Feature, PC score) ===")
print(corr_pc.round(2))

# Vẽ biểu đồ loadings (3 PC)
...
# 5) TRỰC QUAN KMEANS
# =========================
pca_scatter_3d(X_scaled, df_cf_l1["KMeans_Label"], f"KMeans Clusters (K={len(cluster_count)}) — Demographics")
