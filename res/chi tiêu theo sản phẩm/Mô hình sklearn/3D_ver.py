...
OUT_DIR = BASE_DIR / "output_products_clr_3dhier"
...
def pca_scatter_3d(X_scaled, labels, title):
    """Trực quan PCA 3D với loadings và tương quan."""
    ...
    sc = ax.scatter(
        ...
        c=labels, cmap=cmap_custom, s=25, alpha=0.7
    )
    ...
    # In tương quan Feature ↔ PC scores
    scores = pd.DataFrame(X_pca, columns=["PC1_score","PC2_score","PC3_score"])
    corr_pc = pd.DataFrame(X_scaled).join(scores).corr().iloc[:-3, -3:]
...
def pca_scatter(X_scaled, labels, title):
    ...
    plt.tight_layout(); plt.show()
def pca_scatter_3d_centroids(cf_summary_l2, title):
    """PCA 3D chỉ trên các centroid của CF L2."""
    cents = np.vstack(cf_summary_l2["Centroid_scaled"].values)
    labels = cf_summary_l2["L2_Label"].values

    pca = PCA(n_components=3, random_state=RANDOM_STATE)
    cents_pca = pca.fit_transform(cents)
    var = pca.explained_variance_ratio_ * 100
    ...
    sc = ax.scatter(
        cents_pca[:, 0], cents_pca[:, 1], cents_pca[:, 2],
        c=labels, cmap=cmap_custom, s=80, edgecolors="k"
    )
    ax.set_xlabel(f"PC1 ({var[0]:.1f}%)")
    ax.set_ylabel(f"PC2 ({var[1]:.1f}%)")
    ax.set_zlabel(f"PC3 ({var[2]:.1f}%)")
    ax.set_title(title + " (Centroids only)")
    plt.colorbar(sc, label="L2 Cluster")
    plt.tight_layout(); plt.show()
...
pca_scatter_3d(X_scaled, df_cf_l1["CF_Label"], "BIRCH Micro-Clusters (CLR(mix) + log_total)")
...
# 6) TRỰC QUAN
# =========================
def pca_scatter_3d_cfL2_kmeans(cf_l2, km, title):
    """PCA 3D trên centroids của CF L2, tô màu theo nhãn KMeans."""
    cents = np.vstack(cf_l2["Centroid_scaled"].values)
    labels = km.labels_   # nhãn KMeans gán cho mỗi centroid L2
    ...
    sc = ax.scatter(
        cents_pca[:, 0], cents_pca[:, 1], cents_pca[:, 2],
        c=labels, cmap= cmap_custom, s=120, edgecolors="k"
    )
    ax.set_xlabel(f"PC1 ({var[0]:.1f}%)")
    ax.set_ylabel(f"PC2 ({var[1]:.1f}%)")
    ax.set_zlabel(f"PC3 ({var[2]:.1f}%)")
    ax.set_title(title + " (KMeans on CF L2)")
...
#mỗi chấm là 1 khách hàng
pca_scatter_3d(X_scaled, df_cf["KMeans_Label"], f"KMeans Clusters (K={len(cluster_count)}) — CLR(mix) + log_total")
#Mỗi chấm là 1 Supper-CF l2
pca_scatter_3d_cfL2_kmeans(cf_l2_for_km, km, 
    f"KMeans trên Super-CF L2 (K={K_BEST})")
