...
from sklearn.preprocessing import StandardScaler, RobustScaler  # [ADD] RobustScaler (tuỳ chọn)
...
TRY_K_LIST = [3, 4, 5, 6]  # [CHANGED] mở rộng thêm K=3 để phù hợp dữ liệu có trục “quy mô”

# BIRCH (micro-clusters)
BIRCH_THRESHOLD = 0.3     # [CHANGED] bạn đang dùng 0.3 → giữ mặc định 0.3
MIN_CF_SIZE = 3           # [KEEP] lọc CF quá nhỏ trước khi KMeans; test thực tế cho kết quả tốt hơn 1
...
# Scale
USE_ROBUST_SCALER = False  # [ADD] nếu outlier nặng, bật True
RANDOM_STATE = 42

# Trọng số KMeans cho CF
WEIGHT_EXP = 1.0  # [ADD] 1.0 = dùng N; >1.0 phạt mạnh CF nhỏ (vd 1.5–2.0 nếu giữ nhiều CF nhỏ)

# Tuỳ chọn xử lý đa cộng tuyến “quy mô”
DROP_TOTAL_SPENT_IF_DUP = True   # [ADD] tự động bỏ Total_Spent nếu trùng mạnh với tổng Mnt*
DUP_CORR_THRESHOLD = 0.95        # [ADD] ngưỡng tương quan cao
...
def load_and_prepare(df_path, features, log_cols=None, apply_log=False):
    ...
    # [ADD] Kiểm tra đa cộng tuyến “quy mô”: Total_Spent vs sum(Mnt*)
    features_use = features.copy()
    if DROP_TOTAL_SPENT_IF_DUP and "Total_Spent" in features_use:
        sum_mnt = df[FEATURES_PRODUCTS].sum(axis=1)
        if df["Total_Spent"].corr(sum_mnt) >= DUP_CORR_THRESHOLD:
            print(f"[INFO] Total_Spent ~ sum(Mnt*), corr≈{df['Total_Spent'].corr(sum_mnt):.3f} → drop Total_Spent.")  # [ADD]
            features_use = [f for f in features_use if f != "Total_Spent"]

    X_raw = df[features_use].copy()   # bản gốc để báo cáo
    X = X_raw.copy()
    ...
    # [CHANGED] cho phép RobustScaler khi có outlier
    scaler = RobustScaler() if USE_ROBUST_SCALER else StandardScaler()
    X_scaled = scaler.fit_transform(X)
    return df, X_raw, X, X_scaled, scaler, features_use  # [CHANGED] trả ra features_use
...
def kmeans_weighted_on_cf_scaled(cf_summary, n_clusters, random_state=42, weight_exp=1.0):
    cents = np.vstack(cf_summary["Centroid_scaled"].values)
    weights = (cf_summary["N"].values.astype(float)) ** weight_exp  # [CHANGED] luỹ thừa trọng số
    km = KMeans(n_clusters=n_clusters, random_state=random_state, n_init=20)  # [CHANGED] n_init=20 ổn định hơn
...
# 1) Đọc & tiền xử lý
# =========================
df, X_raw, X_log, X_scaled, scaler, FEATURES_USED = load_and_prepare(  # [CHANGED] lấy FEATURES_USED
    INPUT_CSV, FEATURES, LOG_COLS, APPLY_LOG_TRANSFORM
)
print(f"[INFO] FEATURES_USED: {FEATURES_USED}")  # [ADD] log feature thực dùng

# =========================
# 2) BIRCH → CF
# =========================
df, cf_summary = build_cf_consistent(df, X_raw, X_log, X_scaled, FEATURES_USED, threshold=BIRCH_THRESHOLD)
...
# =========================
# PCA diagnostics (loadings/đóng góp/tương quan)
# =========================
X_pca, explained, pca_model, loadings_df, contrib_pc_df, corr_df = pca_fit_and_print(X_scaled, FEATURES_USED)
ax = loadings_df.plot(kind="bar", figsize=(9,4))
ax.set_title("PCA Loadings"); ax.set_ylabel("Loading")
plt.xticks(rotation=45, ha="right"); plt.tight_layout(); plt.show()

# =========================
# Heatmap CF (mean raw, kèm N, phân trang)
# =========================
# cf_no_extra = cf_summary.set_index("CF_Label")[FEATURES_USED]
...
best = None
for k in TRY_K_LIST:
    km_big = kmeans_weighted_on_cf_scaled(cf_summary_big, k, random_state=RANDOM_STATE, weight_exp=WEIGHT_EXP)  # [CHANGED]
...
# =========================
kmeans_mean   = df.groupby("KMeans_Label")[FEATURES_USED].mean().sort_index()
kmeans_median = df.groupby("KMeans_Label")[FEATURES_USED].median().sort_index()
kmeans_std    = df.groupby("KMeans_Label")[FEATURES_USED].std().sort_index()
...
global_mean = df[FEATURES_USED].mean()
global_std  = df[FEATURES_USED].std().replace(0, np.nan)
...
# =========================
# 5) Product-mix % theo cụm (chỉ dùng để đọc hiểu)
# =========================
prod_sum = df[FEATURES_PRODUCTS].sum(axis=1).replace(0, np.nan)
...
# =========================
for feat in [f for f in ["Income", "Total_Spent"] if f in FEATURES_USED]:  # [FIX] chỉ vẽ nếu feature đang dùng
